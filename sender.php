	...
	foreach ($text as $temp) {
		if (preg_match("/{{(?:vandal|IPvandal)\|(.+?)}}/", $temp, $m)) {
			...
			if (preg_match("/^\* 处理：$/m", $temp) || preg_match("/^\* 处理：<!-- 非管理員僅可標記已執行的封禁，針對提報的意見請放在下一行 -->$/m", $temp)) {
				...
				$checkdup []= $user;
				$message = '#VIP <a href="https://zh.wikipedia.org/wiki/Wikipedia:当前的破坏#.7B.7Bvandal.7C'.str_replace(["%", "~"], [".", ".7E"], rawurlencode(str_replace(" ", "_", $user))).'.7D.7D">'.$user.'</a>';
				if (isset($list[$user])) {
...
function RFPPHandler(){
	...
	foreach ($text as $temp) {
		if (preg_match("/===(.+?)===/", $temp, $m)) {
			...
			if (!preg_match("/({{RFPP\||{{y\||拒絕|拒绝|錯誤報告|(?<!请求|請求)(半|全|白紙)?(保護|保护)\d+(日|周|週|個月)|不是(编辑战|編輯戰)|完成|Done)/i", $temp)) {
