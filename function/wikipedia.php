...
function PageStatusHandler($type, $hashtag, $page, $regex)
{
	...
	foreach (PageMatchList($page, $regex) as $section) {
		...
		} else if (in_array($type, ["rfrpatrol", "rfrrollback", "rfripbe", "rfrautoreview", "rfrconfirm", "rfrmms", "rfrtranswiki", "rfrawb", "rfrflood"])) {
			$fragment = 'User:' . $section["page"];
		} else if ($type === "uaa") {
			$fragment = '用户报告';
		} else if ($type === "affp") {
			$fragment = ($section['page'] === '無標題' ? '' : $section['page']);
		} else if ($type === "drv") {
